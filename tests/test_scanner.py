...
class TestSesameScannerDetection:
    ...
    def test_detected_devices_populated_on_detection(
        ...
        mfr_data = _make_manufacturer_data(
            const.ProductModels.SESAME5, device_uuid
        )
    ...
    def test_detection_ignores_unsupported_model(
        self, mocker: MockerFixture
    ) -> None:
        """Unsupported model IDs are silently ignored."""
        device_uuid = UUID("01234567-89ab-cdef-0123-456789abcdef")
        mfr_data = struct.pack(
            "<HB16s", 999, 1, device_uuid.bytes
        )
        adv_data = mocker.Mock()
        adv_data.manufacturer_data = {const.COMPANY_ID: mfr_data}
        ble_device = mocker.Mock(address="AA:BB:CC:DD:EE:FF")
    ...
    def test_detection_invokes_registered_callback(
        self, mocker: MockerFixture
    ) -> None:
        ...
        expected_adv = protocol.SesameAdvertisementData.from_manufacturer_data(
            mfr_data
        )
...
class TestSesameScannerCallbackRegistration:
    ...
    async def test_detected_devices_generator_yields_device(
        ...
        device_uuid = UUID("01234567-89ab-cdef-0123-456789abcdef")
        expected_adv = _make_sesame_adv_data(
            const.ProductModels.SESAME5, device_uuid
        )
...
class TestSesameScannerStartStop:
    ...
    @pytest.mark.asyncio
    async def test_start_clears_devices_and_starts(
        self, mocker: MockerFixture
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_stop_stops_scanner(
        self, mocker: MockerFixture
    ) -> None:
...
class TestSesameScannerFind:
    ...
    async def test_find_device_by_filter_returns_match(
        ...
        mocker.patch.object(
            scanner.SesameScanner, "start", new=mocker.AsyncMock()
        )
        mocker.patch.object(
            scanner.SesameScanner, "stop", new=mocker.AsyncMock()
        )
    ...
    async def test_find_device_by_address_case_insensitive(
        ...
        mock_find = mocker.patch.object(
            scanner.SesameScanner, "find_device_by_filter",
            new=mocker.AsyncMock(return_value=None),
        )

        await scanner.SesameScanner.find_device_by_address(
            "AA:BB:CC:DD:EE:FF"
        )
    ...
    @pytest.mark.asyncio
    async def test_discover_returns_all_detected(
        self, mocker: MockerFixture
    ) -> None:
        """Returns all detected devices after scanning period."""
        mock_scanner_instance = mocker.Mock()
        mock_scanner_instance.detected_devices = {
            "AA:BB:CC:DD:EE:FF": mocker.Mock()
        }
        mocker.patch.object(
            scanner.SesameScanner, "__aenter__",
            new=mocker.AsyncMock(return_value=mock_scanner_instance),
        )
        mocker.patch.object(
            scanner.SesameScanner, "__aexit__",
            new=mocker.AsyncMock(return_value=None),
        )
        mocker.patch(
...
class TestSesameScannerDetectedDevicesProperty:
    ...
    def test_detected_devices_returns_copy(
        self, mocker: MockerFixture
    ) -> None:
